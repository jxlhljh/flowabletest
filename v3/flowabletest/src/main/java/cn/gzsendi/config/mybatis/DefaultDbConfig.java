...
@Configuration  
@MapperScan(basePackages = {"cn.gzsendi.**.mapper"},annotationClass = UsingDefaultDB.class, sqlSessionFactoryRef = "defaultSqlSessionFactory")
public class DefaultDbConfig {
	
	private String aliasesPackage = "cn.gzsendi.**.model";
	...
	public DataSource dataSource(){
		...
		//去掉wallFilter(),解决ds.setProxyFilters(Arrays.asList(statFilter(),logFilter(),wallFilter()));的问题
		ds.setProxyFilters(Arrays.asList(statFilter(),logFilter()));
...
//        filter.setStatementPrepareAfterLogEnabled(false);
    ...
    public SqlSessionFactory sqlSessionFactory(@Qualifier("defaultDataSource") DataSource dataSource) throws Exception {
        ...
        sqlSessionFactoryBean.setMapperLocations(resolver.getResources("classpath*:cn/gzsendi/**/mapper/xml/*.xml"));  
        sqlSessionFactoryBean.getObject().getConfiguration().setMapUnderscoreToCamelCase(true);//开启驼峰支持
        sqlSessionFactoryBean.getObject().getConfiguration().setCallSettersOnNulls(true);;//返回结果为map时空值字段也要返回
